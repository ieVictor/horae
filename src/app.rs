...
impl App {
    pub fn new(conn: Connection) -> Result<Self> {
        let today_secs = crate::db::study_block::today_total_secs(&conn)?;
        Ok(Self {
            conn,
                        ...
                        KeyCode::Enter => {
                            if let Some(Overlay::StartSelector(s)) = self.overlay.take() {
                                if let Some(subj) = s.selected() {
                                    ...
                                    let block =
                                        crate::db::study_block::create(&self.conn, &subject_id)?;
                ...
                match action {
                    ...
                    Some(Action::RequestStart) => {
                        ...
                        let initial_questions = if let Some(first) = subjects.first() {
                            crate::db::question::find_open_for_subject(&self.conn, &first.id.0)?
                        } else {
                            vec![]
                        };
                    ...
                    Some(Action::FetchSubjectSessions(id)) => {
                        let sessions = crate::db::subject::find_blocks(&self.conn, &id, 10)?;
                    ...
                    Some(Action::CreateTask { title, description, priority }) => {
                        crate::db::task::create(&self.conn, &title, description.as_deref(), priority)?;
                        self.refresh_tasks()?;
                    }
                    Some(Action::DeleteTask(id)) => {
                        crate::db::task::delete(&self.conn, &id)?;
                        self.refresh_tasks()?;
                    }
                    Some(Action::ToggleTask(id)) => {
                        crate::db::task::toggle_status(&self.conn, &id)?;
                        self.refresh_tasks()?;
                    }
                    ...
                    Some(Action::DeleteSubject(id)) => {
                        crate::db::subject::delete(&self.conn, &id)?;
                        self.refresh_subjects()?;
                    }
                    ...
                    Some(Action::ToggleQuestionResolved { id, block_id, resolved }) => {
                        if resolved {
                            crate::db::question::resolve(&self.conn, &id, None, Some(&block_id))?;
                        } else {
                            crate::db::question::reopen(&self.conn, &id)?;
                        }
                    }
                    Some(Action::AnswerQuestion { id, answer, block_id }) => {
                        crate::db::question::resolve(&self.conn, &id, Some(&answer), Some(&block_id))?;
                    }
                    Some(Action::SaveCapturedQuestions { questions, subject_id, block_id }) => {
                        for text in &questions {
                            crate::db::question::create(
                                &self.conn,
                                text,
                                &subject_id,
                                Some(&block_id),
                    ...
                    Some(Action::OpenSubjectQuestions(id)) => {
                        let questions = crate::db::question::find_all_for_subject(&self.conn, &id)?;
                        if let Some(Overlay::Subjects(c)) = &mut self.overlay {
                    ...
                    Some(Action::ToggleSubjectQuestionResolved { id, resolved }) => {
                        if resolved {
                            crate::db::question::resolve(&self.conn, &id, None, None)?;
                        } else {
                            crate::db::question::reopen(&self.conn, &id)?;
                        }
                    ...
                    Some(Action::AnswerSubjectQuestion { id, answer }) => {
                        crate::db::question::resolve(&self.conn, &id, Some(&answer), None)?;
                        self.refresh_subject_questions()?;
                    }
    ...
    fn update_selector_questions(&mut self) -> Result<()> {
        ...
        if let Some(id) = subject_id {
            let questions = crate::db::question::find_open_for_subject(&self.conn, &id)?;
            if let Some(Overlay::StartSelector(s)) = &mut self.overlay {
